...
#include "state.h"
/*
 * Some machines (including VMS vaxen) don't have an alloca call, so use
 * malloc/free instead if NO_ALLOCA is defined
 */
...
#else
  errs *errp = (errs *) malloc(sizeof(errs) + ntokens * sizeof(short));
#endif
  ...
  if (errp->nerrs)
    {
	 ...
	 a permanent errs structure for this state, to record them.  */
      ...
      err_table[state] = (errs *) b_allocate ((unsigned int)i);
