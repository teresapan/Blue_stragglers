	integer i, NXY, key
        integer sm_device
	parameter(NXY=20)
	double precision x(NXY),y(NXY),err(NXY)
        double precision pt(NXY)
...
c
	...
	call sm_window(2,2,2,1,2,1)
	call sm_limits(0.0D0,3.0D0,0.0D0,500.0D0)
	call sm_ticksize(-1.0D0,0.0D0,0.0D0,0.0D0)
	call sm_box(1,2,3,3)
        call sm_gflush
	call sm_alpha
	...
	call sm_erase
	call sm_limits(-1.0D0,22.0D0,0.0D0,500.0D0)
	call sm_ticksize(0.D0,0.D0,0.D0,0.D0)
	call sm_box(1,2,0,0)
	call sm_angle(45.0D0)
	call sm_ptype(pt,NXY)
	call sm_points(x,y,NXY)
	call sm_angle(0.0D0)
	call sm_lweight(1.0D0)
	call sm_expand(2.0D0)
	...
	call sm_lweight(3.0D0)
	call sm_histogram(x,y,NXY)
	call sm_lweight(1.0D0)
	call sm_ltype(2)
	call sm_conn(x,y,NXY)
	call sm_ltype(0)
	call sm_relocate(10.0D0,100.0D0)
	call sm_label('Hello ')
	call sm_dot
	call sm_draw(20.0D0,100.0D0)
